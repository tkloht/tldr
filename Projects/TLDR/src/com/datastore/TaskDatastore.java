	...
	private class CreateFakeTasksTask extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... v) {
			...
			try {
				...
				// gs_t1.addBaseData("Fahre langsamer als 50km/h",
				// "null").addCondition("Speed", "leq", "50").addReward("xp",
				// "2000");
				// gs_t2.addBaseData("Radiofrequenz an Ampel wechseln",
				// "traffic_lights_entry").addCondition("Speed", "leq",
				// "10").addCondition("radio_freq", "change",
				// "null").addReward("xp", "4000");
				// gs_t3.addBaseData("Schalte Warnblinklicht an Baustelle ein",
				// "null").addCondition("hazards_light", "eq",
				// "on").addReward("xp", "3000");
				// gs_t4.addBaseData("Melde Stoppschilder",
				// "stop_sign_entry").addCondition("handbreak", "eq",
				// "on").addReward("xp", "1600");
				// gs_polyline_t4 =
				...
				// "xp", 100);
				// gs1_tDemo.addBaseData("Schalte in den 1. Gang",
				// "null").addCondition("CurrentGear", "eq",
				// "1").addReward("xp", "2000");
				// two_2conditions.addBaseData("Fahre 50km/h im 4 Gang.",
				// "null").addCondition("CurrentGear", "eq",
...
//						.addReward("xp", "2000");
				...
				try {
					// String goal_t1=
					// JsonParser.writeNewJsonGoalString(gs_t1.getJsonParse());
					// String goal_t2=
					// JsonParser.writeNewJsonGoalString(gs_t2.getJsonParse());
					// String goal_t3=
					// JsonParser.writeNewJsonGoalString(gs_t3.getJsonParse());
					// String goal_t4=
					// JsonParser.writeNewJsonGoalString(gs_t4.getJsonParse());
					// String goal_demo=
					// JsonParser.writeNewJsonGoalString(gs1_tDemo.getJsonParse());
					// String goal_demo2=
					...
					// Goal().setJsonString(goal_t2)).execute();
					// Goal g_t3=goal_service.insertGoal(new
					// Goal().setJsonString(goal_t3)).execute();
					// Goal g_t4=goal_service.insertGoal(new
					// Goal().setJsonString(goal_t4)).execute();
					// Goal g1_demo=goal_service.insertGoal(new
					// Goal().setJsonString(goal_demo)).execute();
					// Goal g2_demo=goal_service.insertGoal(new
...
//					tt_goals.add(tt3.getId());
					...
					// "Umrunden Sie den Monbijou Park und melden Sie Unregelmï¿½ï¿½ï¿½ï¿½igkeiten. "+
					...
					// "Fahren Sie dazu ï¿½ï¿½ber die Lindauer Allee und wechseln sie die Radiofrequenz, wenn Sie an einer Ampel stehen.")
					// .setGeoLat(52.574211).setGeoLon(13.349095
					// ).setGoals(goals_t2)).execute();
					// service.insertTask(new
					// Task().setTitle("Untersuchung am Salzufer")
					...
					// "Fahren das Salzufer entlang und markieren Sie Baustellen durch kurzes Betï¿½ï¿½tigen vom Warnblinker")
					// .setGeoLat(52.518493).setGeoLon(13.321928).setGoals(goals_t3)).execute();
					// service.insertTask(new
					// Task().setTitle("Spuren der Vergangenheit")
					// .setDescription("Angeblich existieren an der angegebenen Route noch Relikte aus vergangenen Tagen. "+
					// "Melden Sie diese durch anziehen der Handbremse, wÃ¤hrend Sie davor stehen.")
					...
					// service.insertTask(new
					// Task().setTitle("Besuche den GroÃŸmeister")
					// .setDescription("Der GroÃŸmeister des Ministy of Freedom will sich sehen. Reise zu ihm!")
