...
@Api(name = "goalendpoint", namespace = @ApiNamespace(ownerDomain = "tldr.com", ownerName = "tldr.com", packagePath = ""))
public class GoalEndpoint {
	...
	public Goal removeGoal(@Named("id") Long id) {
		EntityManager mgr = getEntityManager();
		Goal goal = null;
		try {
			goal = mgr.find(Goal.class, id);
	...
	private boolean containsGoal(Goal goal) {
		...
		try {
			Goal item = mgr.find(Goal.class, goal.getId());
			if (item == null) {
				contains = false;
			}
		} finally {
